...
Yocto는 이 세 가지를 구조적으로 해결한다: 레시피가 곧 시스템 정의라 재현되고,
apt가 없으니 덮어쓸 주체가 없고, 필요한 것만 들어간다.
...
Yocto **scarthgap**(4.0 LTS, ~2028 지원)을 맞췄다. 커널이 같아야 기존 BSP에서 검증한
커널 패치들(VI5 픽스 등)을 그대로 가져올 수 있다 — 이식이지 재작성이 아니다.
...
Yocto를 한 번도 안 써본 팀원이 혼자 빌드까지 갈 수 있도록 온보딩 가이드를 함께 작성했다 —
recipe/layer/bitbake를 주방 비유로 풀고, "지금 다 이해할 필요 없음" 링크를 곳곳에 심었다.
포팅은 혼자 했지만 운영은 팀이 하게 될 것이므로.
