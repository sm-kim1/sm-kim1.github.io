...
JetPack 대비 **디스크 67% · 패키지 86% · 메모리 79% 축소**, 누가 빌드해도 같은 이미지가 나오는 체계를 만들었다.
...
Yocto로 옮기면 이 문제들이 같이 풀린다. 레시피에 적힌 대로만 이미지가 만들어지니
재현이 되고, 필요한 패키지만 들어간다.
...
Yocto **scarthgap**(5.0 LTS, ~2028 지원)을 맞췄다. 커널 버전이 같아야 기존 BSP에서
...
- JetPack 대비 **디스크 67% · 패키지 86% · 유휴 메모리 79% 감소** — 같은 보드 · 같은 커널에서 실측 (7.9→2.6GB, 1942→271개, 1430→296MB)
- 레시피 = 시스템 정의: 누가 빌드해도 같은 이미지
- 산출물에 tegraflash 패키지(`doflash.sh` 포함)까지 — 빌드에서 플래시까지 한 흐름
- 부팅 시간 systemd 62 → 6.2초 (전원 인가부터 로그인까지 약 30 → 18.8초)
- NVMe 검출 19/19회, 콜드 파워사이클 10/10 — 플래시 · 부팅 신뢰성 확보

## 부팅 시간과 플래시 신뢰성

최소화 이미지가 부팅되고 나서 시간을 재 보니 systemd 기준 62초였다. 원인은 세 겹이었다.
`networkd-wait-online`이 네트워크를 기다리며 21.6초를 먹었고(마스크), 시리얼 콘솔 115200bps로
커널 로그를 전부 쏟아내느라 3.3초(`quiet loglevel=4`), UEFI가 키 입력을 5.2초 기다렸다
(`Timeout=0`을 dtbo에 주입). 합쳐서 6.2초.

플래시 쪽은 `nvme0n1`이 잡힐 때도 있고 안 잡힐 때도 있는 PCIe 링크 트레이닝 문제였다. 커스텀
UEFI를 복원하고 `nvidia,link_up_to` 대기값을 늘리고 initrd 대기를 15초에서 90초로 올려
콜드 파워사이클 10/10, 웜 포함 19/19로 실패 0을 만들었다.

하나 더, 세 번 재부팅하면 UEFI Shell로 떨어지는 잠복 결함이 있었다. `/etc/nv_boot_control.conf`와
`xxd`가 이미지에 없어 `nvbootctrl verify`가 실행되지 않았고, UEFI 재시도 카운터가 소진되면서
슬롯이 무효 처리되는 경로였다. 옛 이미지에서도 같은 결함이 있었지만 플래시가 QSPI를 지워
카운터가 리셋된 덕에 드러나지 않았을 뿐이다. conf 생성과 tarball에 xxd 추가로 고치고 재부팅
5회로 검증했다.
