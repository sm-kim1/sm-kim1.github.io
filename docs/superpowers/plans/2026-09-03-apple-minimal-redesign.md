...
- [ ] `_sass/portfolio.scss`에 스펙 §2 토큰(`:root`, dark media), base, `.nav`, `.hero`, `.stats`, `.block`/`.sec-head`, `.card`, `.how`, `.rows`, `.chip`, `.pill`, `footer`, `.lightbox`, reduced-motion · reduced-transparency · 760px 미디어 쿼리 작성. 시안 아티팩트 CSS를 기준으로 옮기되 Pretendard 링크는 레이아웃 head에.
...
- [ ] `index.html`: front matter `layout: default`, `permalink: /`. 본문은 시안 마크업 그대로(스펙 §3), 링크는 `relative_url`로 실제 경로.
- [ ] `_includes/footer.html`: `<footer class="site-footer wrap">` 안에 면책 + ©. `data_file` 없을 때 대비 `site.data.data_ko` 직접 참조.
...
- [ ] `.case` 스타일: 스펙 §5 (본문 680px, h2 22px 700, 표 · code · pre · blockquote · img 규칙).
...
- [ ] IBM Plex 링크 → Pretendard 링크. `<style>` 색상 teal · terra → 스펙 §6 값. `.mono`는 `ui-monospace` 스택.
